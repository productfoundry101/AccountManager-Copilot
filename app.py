...
_DOC_NAMES = {
    ...
    "battlecard_keystone": "Keystone battlecard", "battlecard_bloom": "Bloom battlecard",
...
def answer_sources(calls: list[dict]) -> list[str]:
    ...
    AE sees 'Accounts, Opportunities, Bloom battlecard' rather than a list of
    tool names."""
    seen, out = set(), []
    for c in calls:
        if c.get("gated"):
...
_CITE_MAP = {
    ...
    "battlecard_bloom": "Bloom battlecard", "battlecard_keystone": "Keystone battlecard",
    "objection_handling": "objection handling", "pricing_cheatsheet": "pricing guide",
    "customer_case_studies": "case studies",
}
...
with st.sidebar:
    ...
    ae_email = st.selectbox("Signed in as", aes, format_func=lambda e: labels[e],
                            index=aes.index("lena.koehler@humbl.de")
                            if "lena.koehler@humbl.de" in aes else 0)
