...
def check_usage_trend(account_id):
    ...
    AE research (Lena): 'the thing the support tickets are saying that the
...
def check_competitors(account_id):
    """AE research (Marcus): 'if somebody could just tell me this customer is
    also evaluating Bloom and here is the relevant page from the battlecard'."""
    out = []
    seen = set()
    for a in db.activities_for(account_id, limit=500):
        ...
        for comp in COMPETITORS:
            if comp in text and comp not in seen:
                ...
                out.append(_sig(
                    ...
                    f"battlecard: {comp} (knowledge/0{3 if comp=='keystone' else 4}_battlecard_{comp}.md)",
                ))
    return out


def check_quiet(account_id):
    """AE research (Thomas): 'here's what changed since last time you looked'.
    ...
    if gap is not None and gap > QUIET_DAYS:
        return [_sig(
            ...
            "playbook s8 + AE research (Lena): the quietly slipping deal",
        )]
    return []


def check_reference_match(acct, competitor_signals):
    """AE research (Sofia): 'who else have we sold to that looks like this
    ...
    return [_sig(
        ...
        "customer_case_studies + AE research (Sofia): 'who else looks like this customer'",
    )]
...
SIGNAL_GLOSSARY = {
    ...
    "competitor_mention": "Bloom or Keystone named in activity subject/summary "
