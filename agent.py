...
AE_NAMES = {
    "lena.koehler@humbl.de": "Lena Köhler",
    "marcus.byrne@humbl.de": "Marcus Byrne",
    "sofia.alvarez@humbl.de": "Sofia Alvarez",
    "thomas.weber@humbl.de": "Thomas Weber",
    "ines.dubois@humbl.de": "Ines Dubois",
}


def system_prompt(ae_email: str) -> str:
    ae_name = AE_NAMES.get(ae_email, ae_email)
    return f"""You are the AE Copilot, an internal assistant for Humbl Account \
...
tickets, risk signals, or a document (e.g. [source: Bloom battlecard], \
...
- When a competitor (Bloom, Keystone) appears in account data, read the relevant \
...
- When the user raises a SPECIFIC customer objection ("Bloom is cheaper", "your \
